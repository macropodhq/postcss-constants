...
module.exports = postcss.plugin('postcss-local-vars', function (opts) {
    ...
    return function (css) {
        css.eachInside(function (node) {
            if (node.type === 'decl') {
                node.value = strip(node.value, node);
            }

            if (node.type === 'atrule') {
              node.params = strip(node.params, node);
            }
        });
    };
});
